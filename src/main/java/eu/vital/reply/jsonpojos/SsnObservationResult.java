...
public class SsnObservationResult {
    ...
    protected boolean declaredProperty(String name, Object value) {
        switch (name) {
            case "type":
                ...
                return true;
            case "ssn:hasValue":
                if (value instanceof SsnHasValue) {
                    setSsnHasValue(((SsnHasValue) value));
                } else {
                    throw new IllegalArgumentException(("property \"ssn:hasValue\" is of type \"eu.vital.reply.jsonpojos.SsnHasValue\", but got "+ value.getClass().toString()));
                }
                return true;
            default:
                return false;
    ...
    public<T >T get(String name) {
        Object value = declaredPropertyOrNotFound(name, SsnObservationResult.NOT_FOUND_VALUE);
        if (SsnObservationResult.NOT_FOUND_VALUE!= value) {
            return ((T) value);
        } else {
    ...
    public SsnObservationResult with(String name, Object value) {
        if (!declaredProperty(name, value)) {
            getAdditionalProperties().put(name, ((Object) value));
        }
        return this;
